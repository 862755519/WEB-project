  ...
  <div class="tab-cart">
    ...
    <van-checkbox-group @change="toggle" class="card-goods" v-model="checkedGoods">
      <div v-for="(item, i) in goods" :key="i" class="card-goods__item">
        ...
        <van-card @click.stop="go_detal(item.goodsId)" 
         :title="item.goodsName" :price="item.price" :num="item.number" :thumb="item.picUrl">
          <div slot="desc">
          ...
          <div slot="footer" v-if="isEditor">
            <van-stepper v-model="item.number"  disabled   @change="stepperEvent(item,arguments)" disableInput/>
          </div>
    ...
    <van-submit-bar
      ...
      @submit="cartSubmit"
    >
      <van-checkbox v-model="allCheckedStatus" @change="setCheckAll(allCheckedStatus)" style="padding: 0 10px;">全选</van-checkbox>
    </van-submit-bar>
  </div>
</template>

<script>
import { Checkbox, CheckboxGroup, Card, SubmitBar, Stepper, Tag,NavBar,Dialog } from 'vant';
...
export default {
  ...
  methods: {
    ...
    go_detal(goodsId){
      ...
      if(!this.isEditor){
           // alert(goodsId)
      this.$router.push('items/detail/'+goodsId+'?indexBreak=2');
      }
    },
    ...
    cartSubmit(data) {
      ...
      if (this.isEditor) {
        ...
        this.$router.push('/order/checkout?car=1');   
      }
    },
    // 单选择
    select(){
      if(this.AllGoods.length > this.checkedGoods.length){
        this.allCheckedStatus = false;
      }else{
        this.allCheckedStatus = true;
      }
      this.setCheckAll(this.allCheckedStatus);
    },
    // 全选 与 非全选
    setCheckAll(val) {
      if (this.checkedGoods.length === this.AllGoods.length && val == false) {
        this.checkedGoods = [];
      } else if(val == true){
