<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="活动类型" prop="link">
        <template slot-scope="scope">
            <el-tag v-if="scope.row.type==0">商品</el-tag>
            <el-tag v-if="scope.row.type==1">活动</el-tag>
            <el-tag v-if="scope.row.type==2">链接</el-tag>
        </template>
      </el-table-column>
      ...
      <el-form  ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="center" label-width="80px" style="width: 700px; margin-left:50px;">
        ...
        <el-form-item label="广告类型" prop="type">
          <el-select v-model="dataForm.type" placeholder="请选择">
            <el-option :value="0" label="商品"/>
            <el-option :value="1" label="活动"/>
            <el-option :value="2" label="链接"/>
          </el-select>
        </el-form-item>

        <el-form-item v-if="dataForm.type ==0" label="商品" prop="link">
          <el-input v-model="dataForm.link" placeholder ="请输入商品ID"/>
        </el-form-item>

        <el-form-item v-else-if="dataForm.type ==1" label="活动" prop="link">
        <editor id="text" :init="editorInit" v-model="dataForm.link"/>
        </el-form-item>
        <el-form-item v-else label="链接" prop="link">
          <el-input v-model="dataForm.link" placeholder="请输入链接 例如：http://www.baidu.com"/>
        </el-form-item>
