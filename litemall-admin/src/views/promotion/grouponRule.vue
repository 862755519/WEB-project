<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="团购利润" prop="amount"/>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form
        ...
        <el-form-item label="利润" prop="amount">
      ...
      rules: {
        ...
        validator: (rule, value, callback) => {
          if (/^\d+(\.\d{0,2})?$/.test(value) == false) {
            callback(new Error("请输入一个整数或者小数点两位"));
          } else {
            callback();
          }
        }, trigger: 'change'
        ...
        amount: [{  required: true, message: '团购利润不能为空', trigger: 'blur' },{
        validator: (rule, value, callback) => {
          if (/^\d+(\.\d{0,2})?$/.test(value) == false) {
            callback(new Error("请输入一个整数或者小数点两位"));
          } else {
    ...
    handleDownload() {
      this.downloadLoading = true
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['商品ID', '名称', '首页主图', '折扣', '人数要求', '利润','活动开始时间', '活动结束时间']
          const filterVal = ['id', 'name', 'pic_url', 'discount', 'discountMember', 'amount','addTime', 'expireTime']
